...
const TextArea = React.forwardRef<HTMLTextAreaElement, TextAreaProps>(
  ({ className, error, label, description, helperText, id, ...props }, ref) => {
    ...
    return (
      <div className="w-full">
        ...
        {description && (
          <p id={descriptionId} className="mb-2 text-sm text-zinc-500 dark:text-zinc-400">
            {description}
          </p>
        )}
        <textarea
          ...
          ref={ref}
          aria-describedby={
            [description && descriptionId, helperText && helperId].filter(Boolean).join(' ') ||
            undefined
          }
